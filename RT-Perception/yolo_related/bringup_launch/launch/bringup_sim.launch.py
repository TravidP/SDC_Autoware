...
def generate_launch_description():
    ...
    default_bringup_config = os.path.join(
        ...
        'config',
        'bringup_sim_config.yaml'
    )

    with open(default_bringup_config, 'r') as file:
            bringup_config = yaml.safe_load(file)

    # Extract configuration details for video_tools and object_detection_launch
    video_tools = bringup_config["video_tools"]
    ...
    return LaunchDescription([
        ...
        # Include launch file for video source selection
        IncludeLaunchDescription(
            AnyLaunchDescriptionSource([
                FindPackageShare("video_tools"), '/launch', '/video_source_selection.launch.xml']),
                launch_arguments={
                    'video_source': video_tools["video_source"],
                    'rosbag2_path': video_tools["rosbag2_path"],
                    'rosbag2_topics': video_tools["rosbag2_topics"],
                    'svo_path': video_tools["svo_path"],
                }.items(),
        ),
