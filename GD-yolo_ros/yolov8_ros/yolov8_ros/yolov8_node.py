...
class Yolov8Node(Node):
    ...
    def __init__(self) -> None:
        ...
        if model.endswith('.pt'):
            ...
            yolo_base_model = model.split('_')[-1]
            self.yolo = YOLO(f'{model}/{yolo_base_model}-2xhead.yaml', task="detect").load(f'{model}/{yolo_base_model}.pt')
            self.yolo.state_dict().keys()
            state_dict = torch.load(f'{model}/{yolo_base_model}_lp.pth')
            state_dict.keys()
        ...
        self._specific_image_pub = self.create_publisher(Image, "specific_image_detections", 10)
        ...
        if self.enable:
            ...
            for i in range(len(results)):
                ...
                if results.boxes:
                    ...
                    if (aux_msg.class_id == 80 or aux_msg.class_id == 9):
                        self._specific_image_pub.publish(msg)
                    if aux_msg.class_id == 81:
                        continue
